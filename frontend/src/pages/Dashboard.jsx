...
import {
  LogOut, Search, Filter, Plus, MessageSquare, User, Clock,
  Tag, ChevronLeft, Send, CheckCircle, AlertCircle, ShieldCheck,
} from 'lucide-react'
...
export default function Dashboard() {
  ...
  return (
    <div className="flex h-screen bg-slate-50">
      ...
      {showCreate && <CreateModal onClose={() => setShowCreate(false)} onCreated={fetchTickets} />}
    </div>
  )
}

/* ── Ticket Detail ─────────────────────────────────────── */
function TicketDetail({ ticket, onBack, user }) {
  ...
  const fetchComments = useCallback(async () => {
    try {
      const { data } = await api.get(`/api/tickets/${ticket.id}/comments`)
      setComments(data)
    } catch {
      setComments([])
    }
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        <div className="bg-white rounded-xl border border-slate-200 p-5">
          ...
          <div className="mt-4 flex flex-wrap gap-4 text-sm text-slate-500">
            <span className="flex items-center gap-1"><User size={14} /> Requester: #{ticket.requester_id}</span>
            <span className="flex items-center gap-1"><Tag size={14} /> Priority: {ticket.priority}</span>
            <span className="flex items-center gap-1"><CheckCircle size={14} /> Assignee: {ticket.assignee_id ? `#${ticket.assignee_id}` : 'Unassigned'}</span>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          {comments.map((c) => (
            <div
              key={c.id}
              className={`rounded-xl border p-4 ${c.is_internal ? 'bg-amber-50 border-amber-200' : 'bg-white border-slate-200'}`}
            >
              <div className="flex items-center justify-between mb-1">
                <span className="text-sm font-medium text-slate-700">User #{c.user_id}</span>
...
function CreateModal({ onClose, onCreated }) {
  ...
  const handleCreate = async (e) => {
    ...
    setCreating(true)
    try {
      await api.post('/api/tickets', { subject, description, priority })
      onCreated()
      onClose()
    } finally {
      setCreating(false)
